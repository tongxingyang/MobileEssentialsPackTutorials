	    ...
	    *  <remarks>Combines meshes and generates texture atlases</remarks> */
        ...
        AtlasPackingResult[] __RuntTexturePackerOnly(List<MB_TexSet> distinctMaterialTextures, int _padding)
        {
            ...
            if (distinctMaterialTextures.Count == 1 && _fixOutOfBoundsUVs == false)
            {
                ...
                packerRects[0].atlasX = t == null ? 16 : dmt.t.width;
                packerRects[0].atlasY = t == null ? 16 : dmt.t.height;
                packerRects[0].usedW = t == null ? 16 : dmt.t.width;
                packerRects[0].usedH = t == null ? 16 : dmt.t.height;
            }
            else
            {
                            ...
                            */
                            ...
                            if (LOG_LEVEL >= MB2_LogLevel.debug)
                            {
                                if (LOG_LEVEL >= MB2_LogLevel.trace)
                                {
                                    ...
                                    for (int matIdx = 0; matIdx < tx1.matsAndGOs.mats.Count; matIdx++)
                                    {
                                        DRect samplingRectCannonical =  tx1.matsAndGOs.mats[matIdx].samplingRectMatAndUVTiling;
                                        ...
                                        DRect uvR = tx1.matsAndGOs.mats[matIdx].obUVRectIfTilingSame;
                                        DRect matR = tx1.matsAndGOs.mats[matIdx].materialTiling;
                                        ...
                                        if (!tx1.ts[0].encapsulatingSamplingRect.Encloses(tx1.matsAndGOs.mats[matIdx].samplingRectMatAndUVTiling))
                                        {
                                            sb.AppendFormat("mesh " + tx1.matsAndGOs.mats[matIdx].objName + "\n" +
                                                             " uv=" + uvR + "\n" +
                                            ...
                                            sb.AppendFormat(String.Format("Integrity check failed. "+ tx1.matsAndGOs.mats[matIdx].objName + " Encapsulating rect cannonical failed to contain samplingRectMatAndUVTiling cannonical\n"));
                                        }
                                        /*
